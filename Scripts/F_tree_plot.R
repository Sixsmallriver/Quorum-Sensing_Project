...
data <- read.csv('../Data/example_dataset.csv', sep = ',')
# data.1 <- data %>% filter(cluster==91)
data.1 <- data
data.1$time <- data.1$frames * 0.25
...
data.1 <- data.1 %>% group_by(id) %>% mutate(cid = set_cid(id, daughters),
...
data.mean <- data.1 %>% group_by(cid, did1, did2) %>% summarise(cell.mean = mean(fluo2), 
                                                              first.frame = min(time), 
                                                              lifespan = length(frames)*0.25,
...
plot.tree.lasI <- ggtree(Tree, aes(color=nodes$cell.mean), root.position = root_len, size=0.5) +
  geom_rootedge(size=1, color='blue') +
  geom_nodepoint(size=1) +
  theme_tree2() +
  scale_color_gradientn(name='',colours=c("grey", "lightblue","blue")) +
  theme() +
  xlab("Time (h)");plot.tree.lasI
